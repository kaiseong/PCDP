...
from pcdp.dataset.PCDP_stack_dataset import collate_fn
...
class TrainSCDPWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg:OmegaConf, output_dir = None):
        ...
        self.ema_model: PCDPPolicy = None
    ...
    def run(self):
        ...
        H = cfg.policy.get("H", 16)
        DT = cfg.training.get("dt", 0.05)
        USE_S_EMBED = cfg.policy.get("use_s_offset_embedding", True)

        LAMBDA_PTP = cfg.training.losses.get("lambda_ptp", 0.1)
        LAMBDA_JIT = cfg.training.losses.get("lambda_jit", 0.2)
        TAU_SOFT = cfg.training.losses.get("tau_soft",None)
