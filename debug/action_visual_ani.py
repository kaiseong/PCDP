...
# -------------------------------------------------------------
# 모델 체크포인트 파일의 절대 경로
CHECKPOINT_PATH = "/home/nscl/diffusion_policy/data/outputs/RISE_NoFilter_140/checkpoints/latest.ckpt" 
# 데이터셋 디렉토리의 절대 경로
...
class VisController:
    def __init__(self, total_steps, step_size=1):
        ...
        # 애니메이션 제어
        self.auto_play = False       # True면 자동 재생, False면 수동
        self.pause_counter = 0      # 장면 전환 전 대기 카운터

    def toggle_auto_play(self, vis):
        self.auto_play = not self.auto_play
        status = "ON" if self.auto_play else "OFF"
        print(f"\n[System] Auto Play: {status}")
        return False

    def next_scene(self, vis=None):
    ...
    def next_denoising_step(self, vis=None):
...
def create_action_geometries(trajectory, vis_mode):
    ...
    for i in range(num_steps):
        ...
        if vis_mode == VIS_MODE_SPHERE:
            ...
            sphere.translate(grip_pos)
            geometries.append(sphere)
        
        elif vis_mode == VIS_MODE_FRAME:
            transform = np.eye(4)
            transform[:3, :3] = grip_rot
            transform[:3, 3]  = grip_pos
...
def main():
    ...
    vis.create_window(f"Action Denoising | Episode {episode_idx} | {MODEL_TYPE}", width=1280, height=720)
    ...
    vis.register_key_callback(ord(" "), controller.toggle_auto_play) # Spacebar
    ...
    print("  [N]     : Next scene (Manual)")
    print("  [M]     : Next denoising step (Manual)")
    print("  [Q]     : Quit")
    print("="*50 + "\n")
    ...
    # 애니메이션 속도 설정
    step_delay = 0.03       # Denoising 스텝 간 지연 시간 (초)
    scene_change_delay = 20 # Denoising 완료 후 다음 씬으로 넘어가기 전 대기 프레임 수
    ...
    while is_running:
        ...
        if controller.start_new_prediction:
            ...
            print(f"Scene {controller.current_step} initialized.")

        # =========================================================
        # [AUTO PLAY 로직]
        # =========================================================
        if controller.auto_play and not controller.vis_changed and not controller.start_new_prediction:
            if controller.denoising_step < controller.denoising_total_steps:
                 # Denoising 진행
                 controller.run_denoising_step = True
                 time.sleep(step_delay)
            else:
                # Denoising 완료 시 잠시 대기 후 다음 장면
                controller.pause_counter += 1
                if controller.pause_counter > scene_change_delay:
