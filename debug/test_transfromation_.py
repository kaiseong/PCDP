...
pose_9d = xyz_rot_transform(TF_pose_6d, from_rep="euler_angles", to_rep="rotation_6d", from_convention="XYZ")
actions_7d = np.concatenate([pose_9d, gripper], axis=-1)

# 4단계 normalize
actions_7d[:, :3] = (actions_7d[:, :3] - TRANS_MIN) / (TRANS_MAX - TRANS_MIN) * 2 - 1
actions_7d[:, -1] = actions_7d[:, -1] * 2 - 1
...
actions_7d_tensor = torch.from_numpy(actions_7d).to(device).float()
...
actions_7d_tensor[..., :3] = (actions_7d_tensor[..., :3] + 1) / 2.0 * (trans_max_exp - trans_min_exp) + trans_min_exp
actions_7d_tensor[..., -1] = (actions_7d_tensor[..., -1] + 1) / 2.0 

# 이후 코드에서 NumPy 배열이 필요할 수 있으므로 다시 변환해줍니다.
pred = actions_7d_tensor.cpu().numpy()
...
print(f"1: {pose_eulers[0] - action_sequence_7d[0]}")
print(f"2: {pose_eulers[1] - action_sequence_7d[1]}")